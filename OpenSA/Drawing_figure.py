...
if __name__ == '__main__':
    ...
    # 打开文件  
    with open('x_axis.txt', 'r') as file:  
        # 遍历文件中的每一行  
        for line in file:  
            # 将每一行的内容转换为float型，并添加到列表中  
    ...
    labels = ['Origin 1-BiLiTong', 'Imitation 1:Dongbei', 'Imitation 2:WeFuiry','Imitation 3:TongDe']  # 每个数据集的标签
    x_axis = numbers[0:-1]
    for sublist, color, label in zip(list_of_lists, colors, labels):
        plt.plot(x_axis,sublist, color=color, label=label)
...
# ————————————————
    ...
    plt.ylabel('Raman intensive:(a.u.)')
    plt.legend()
